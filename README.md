...
### Building the NFA 
Thompson's Construction Algorithm is used to build NFA's from the user's input in postfix notation. Both NFA's and Regular expressions have the same computational power, therefore any language that can be recognised by a regex can also be represented as a NFA, and visa versa.  
...
A stack structure is used to store the fragments for the creation of the overall NFA. In Thompson's Construction, all fragments have a single initial state(required for an NFA) and a single final state. I used the following NFA struct  to represent this: 
```go 
type  nfa  struct { 
        initial *state 
        accept *state 
...
| ? | zero or one|![enter image description here](https://github.com/DaireNiC/Regex-NFA-Builder/blob/master/media/zero_or_one..JPG)| 
  ...
  Once the NFA is created, the user can use it to test against an input string.  
...
The NFA is first created using the previously described NFA fragments. The string matching algorithim is then implemented.  When matching aginst an NFA, the input can be in a number of states at a given time. To keep track of the states, an array of pointers to the current states is used. The current set of states are updated as each character of the input string is processed.  If any of the current states are the **accept state** (indicated by symbol == 0 and no arrows coming from it),  the function returns with a true value. 
...
### Running the Tests 
 ...
 -  Navigate to the folder 
```bash 
        cd Regex-NFA-Builder/nfaBuilder 
``` 
 - Execute the tests 
```go 
        go test 
